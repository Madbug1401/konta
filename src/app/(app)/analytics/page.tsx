...
export default async function AnalyticsPage({
  ...
  const cashflowPoints = cashflow.buckets.map((b) => ({ label: b.label, income: Number(b.incomeMinor), expense: Number(b.expenseMinor) }));
    ...
    <div className="mx-auto flex max-w-6xl flex-col gap-6">
      ...
      <section id="analytics-section-overview">
        ...
        <div className="grid grid-cols-2 gap-3 sm:gap-4 lg:grid-cols-4">
          <MetricCard label="Receita" value={overview.income.current} changePercent={overview.income.changePercent} direction={overview.income.direction} />
          <MetricCard label="Despesas" value={overview.expenses.current} changePercent={overview.expenses.changePercent} direction={overview.expenses.direction} invert />
          <MetricCard label="Cash flow" value={overview.cashflow.current} changePercent={overview.cashflow.changePercent} direction={overview.cashflow.direction} />
          <MetricCard
        ...
        {debts.debts.length === 0 ? (
          ...
          <div className="flex flex-col gap-3">
            ...
            <ul className="flex flex-col gap-1.5">
              {debts.debts.map((d) => (
                <li key={d.debtId} className="flex items-center justify-between rounded-lg border border-border px-3 py-2 text-sm">
                  <span className="font-medium text-foreground">{d.creditorName}</span>
                  <span className="tabular-nums text-muted-foreground">{d.remaining} restantes · {d.progressPercent.toFixed(0)}% pago</span>
                </li>
              ))}
            </ul>
          </div>
            ...
            {goals.goals.map((g) => (
              <li key={g.goalId} className="rounded-lg border border-border p-3">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-xs text-muted-foreground">{g.progressPercent.toFixed(0)}%</span>
                </div>
      ...
      <Card id="analytics-section-trends">
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
          <TrendBadge label="Receita" direction={trends.income.direction} />
          <TrendBadge label="Despesas" direction={trends.expenses.direction} invert />
          <TrendBadge label="Cash flow" direction={trends.cashflow.direction} />
          <TrendBadge label="Património" direction={trends.netWorth.direction} />
        </div>
...
function TrendBadge({ label, direction, invert }: { label: string; direction: "up" | "down" | "flat"; invert?: boolean }) {
  const goodDirection = invert ? "down" : "up";
