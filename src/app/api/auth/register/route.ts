...
import { z } from "zod";
import { createUser, findUserByEmail, touchLastLogin } from "@/lib/db/users";
...
export const POST = withErrorHandling("api.auth.register.post", async (request: Request) => {
  ...
  // Registar já inicia sessão de imediato (ver cookie abaixo) — por isso
  // conta como o primeiro "login" da pessoa.
